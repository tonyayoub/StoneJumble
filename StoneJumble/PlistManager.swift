...
let progressFileName:String = "Progress"
...
struct Plist {
    ...
    //this atually replaces the whole file with the new dictionary, or that what i think till now
...
class PlistManager {
    ...
    func saveBoolValue(value:Bool, forKey:String)
    {
        if let plist = Plist(name: progressFileName)
        {
    ...
    func saveIntValue(value:Int, forKey:String)
    {
        if let plist = Plist(name: progressFileName)
        {
